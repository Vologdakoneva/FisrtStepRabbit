                ...
                using (var scope = serviceScopeFactory.CreateScope())
                {
                    ...
                    if (!UserTask.successfully)
                    {
                        ...
                        if ((bool)UserTask.usemail )
                        {
                            ...
                            mail.Subject = configuration.GetConnectionString("SMTPSUBJECT");
                        ...
                        if ((bool)UserTask.usetelegram)
                        {
                            ...
                            Update? update = upd.FirstOrDefault(p => p.Message.Chat.Username.ToUpper() == UserTask.telegram.Replace("@",""));
                            

                            if (update != null)
                            {
                                string Telegamesage = UserTask.DataTask.ToString("dd-MM-yyy") + " " + UserTask.ownertask + " назначил Вам зачачу " +
                                                      "<b>" + UserTask.TextTask + "</b>" + "<br />" +
                                                      "Срок исполнения " + UserTask.DataTaskPlan.ToString("dd-MM-yyy") + "<br />" +
